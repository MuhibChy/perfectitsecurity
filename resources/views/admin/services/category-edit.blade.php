...
<div class="max-w-2xl">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mt-2">Edit: {{ $category->name }}</h1>
    </div>

    <form method="POST" action="{{ route('admin.service-categories.update', $category->id) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="glass-card p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <input type="text" name="color_hex" value="{{ old('color', $category->color ?? '#6366f1') }}" readonly
        ...
        <div class="flex justify-between">
            <form method="POST" action="{{ route('admin.service-categories.destroy', $category->id) }}"
                ...
                @csrf @method('DELETE')
                <button type="submit" class="px-6 py-2 border border-red-300 dark:border-red-700 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-sm {{ $category->services_count > 0 ? 'opacity-50 cursor-not-allowed' : '' }}" {{ $category->services_count > 0 ? 'disabled' : '' }}>
                    Delete
                    @if($category->services_count > 0)
                        <span class="text-xs">({{ $category->services_count }} services)</span>
                    @endif
                </button>
            </form>
            <div class="flex gap-3">
                ...
                <button type="submit" form="edit-form" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">Update Category</button>
            </div>
        </div>
    </form>
</div>
